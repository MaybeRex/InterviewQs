...
const merge = (firstList, firstListLength, secondList, secondListLength) => {
  for (let i = 0; i < firstList.length; i += 1) {
    ...
    if (item < secondList[0]) {
      continue;
    }

    firstList.pop();
    firstList.splice(i, 0, secondList.shift());
  }
  ...
  firstList.splice(firstList.length - secondList.length, secondList.length + 1, ...secondList);
  return firstList;
};
